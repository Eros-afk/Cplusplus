...
void ExibirMenu()
{
	...
	} while (ProcessarEscolha(RetornarEscolha()) != 3);//enquanto o retorno da função RetornarEscolha for diferente de 3 o menu vai aparecer! pois você está solicitando que fique em loop até que escolha seja igual a 3 e resultado seja falso saindo do loop do-while
}
...
int ProcessarEscolha(int TipoEscolha)
{
    ...
    return TipoEscolha; // Retorna o valor de TipoEscolha para indicar que uma opção inválida foi selecionada
}